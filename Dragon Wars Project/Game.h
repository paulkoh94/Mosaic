...
const string DRAGON_DATA = "dragon_data.txt"; //File constant
...
public:
  ...
  */
  ...
  void HatchEgg();
   ...
   // Postconditions - Continually checks to see if player has an egg or a drago\
 n
   void StartGame();
   ...
   // FYI: remove element at front of a vector is m_vectorName.erase(m_vectorNam\
 e.begin());
 ...
 private:
   ...
   int m_countHatchedDragons; //Current number of hatched dragons the player has\
  (for attack)
